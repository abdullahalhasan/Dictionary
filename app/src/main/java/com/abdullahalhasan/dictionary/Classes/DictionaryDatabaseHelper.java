...
public class DictionaryDatabaseHelper extends SQLiteOpenHelper {
    ...
    public long insertData(WordDefinition wordDefinition) {
        ...
        values.put(WORD_COLUMN, wordDefinition.getWord());
        values.put(DEFINITION_COLUMN, wordDefinition.getDefinition());
    ...
    public long updateData(WordDefinition wordDefinition) {
        ...
        return database.update(DICTIONARY_DATABASE, values, WORD_COLUMN+" =?", new String[]{wordDefinition.getWord()});

    }

    public void deleteData(WordDefinition wordDefinition) {
        SQLiteDatabase database=this.getWritableDatabase();
        String queryString="DELETE FROM "+DICTIONARY_DATABASE+" WHERE "+WORD_COLUMN+" = '"+wordDefinition.getWord()+"'";
    ...
    public void initializeDatabaseFortheFirstTime(ArrayList<WordDefinition> wordDefinitions) {
        ...
        for (WordDefinition wordDefinition : wordDefinitions) {
            contentValues.put(WORD_COLUMN, wordDefinition.getWord());
            contentValues.put(DEFINITION_COLUMN, wordDefinition.getDefinition());
